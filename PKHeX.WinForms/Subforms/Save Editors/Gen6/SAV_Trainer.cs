...
public partial class SAV_Trainer : Form
{
    ...
    private void SetBattleChateauGrandDuke(object sender, EventArgs e)
    {
        if (SAV is not SAV6XY xy || xy.SUBE is not SubEventLog6XY sube)
        {
            WinFormsUtil.Alert("Battle Chateau is only available in Pokémon X/Y.");
            return;
        }

        sube.SetChateauGrandDuke();
        WinFormsUtil.Asterisk();
    }
}
